            ...
            <div class="row">
                <div class="col-lg-8 col-12">
                    ...
                    <div class="checkout-form">
                        ...
                        @if (!Auth::check())
                               <p>{{ _lang('You have an account already?') }} <a href="{{ url('/sign_in') }}"
                                    class="btn-link"><b class="text-warning">{{ _lang('Login to your account') }}</b></a>
                            </p>
<p>OR</p>
                            <p>{{ _lang('If you don not have an account?') }} <a href="{{ url('/sign_up') }}"
                                    class="btn-link"><b
                                        class="text-danger">{{ _lang('Please create an account and continue') }}</b></a></p>
                            ...
                            <!--                <input type="text" name="city" placeholder="{{ _lang('City') }}"-->
                            <!--                    required="required">-->
                            <!--                @if ($errors->has('city'))-->
                            <!--                    <div class="invalid-feedback">-->
                            <!--                        {{ $errors->first('city') }}-->
                            ...
                            <!--                <input type="text" name="post_code" placeholder="{{ _lang('Postal Code') }}"-->
                            <!--                    required="required">-->
                            ...
                            <!--                <input type="text" name="address" placeholder="{{ _lang('Address') }}"-->
                            <!--                    required>-->
                            ...
                            <!--                <label><input id="create_account" name="create_account" value="1"-->
                            <!--                        type="checkbox"> {{ _lang('Create an account?') }}</label>-->
                            <!--            </div>-->
                            <!--        </div>-->
                            ...
                            <!--                    placeholder="{{ _lang('Password Confirmation') }}" required="required">-->
                        ...
                        @else
                                            ...
                                            <select name="shipping_address" id="select-shipping-address" required>
                                                @foreach ($address_list as $shipping_address)
                                                    <option value="{{ $shipping_address->id }}"
                                                        ...
                                                        {{ $shipping_address->is_default == 1 ? 'selected' : '' }}>
                                                        {{ $shipping_address->address }}</option>
                                                @endforeach
                                            </select>
                                        </div>
                                    </div>
                                            ...
                                            <select name="billing_address" id="select-billing-address" required>
                                                @foreach ($address_list as $billing_address)
                                                    <option value="{{ $billing_address->id }}"
                                                        ...
                                                        {{ $billing_address->is_default == 1 ? 'selected' : '' }}>
                                                        {{ $billing_address->address }}</option>
                                                @endforeach
                                            </select>
                                        </div>
                                    </div>
                            ...
                            <a href="" id="add_new_address"><i class="fa fa-plus"></i>
                                {{ _lang('Add New Address') }}</a>
                                    ...
                                    <div class="row">
                                        <div class="col-md-12">
                                            <input type="text" name="name" value="{{ old('name') }}"
                                                class="form-control" placeholder="{{ _lang('Name') }}" required>
                                        </div>
                                        ...
                                        <div class="col-md-6">
                                            <input type="text" name="city" value="{{ old('city') }}"
                                                class="form-control" placeholder="{{ _lang('City') }}" required>
                                        </div>

                                        <div class="col-md-6">
                                            <input type="text" name="post_code" value="{{ old('post_code') }}"
                                                class="form-control" placeholder="{{ _lang('Post Code') }}" required>
                                        </div>
                ...
                <div class="col-lg-4 col-12">
                    <div class="order-details">
                                    ...
                                    <li>
                                        <form id="apply_coupon" action="{{ url('/apply_coupon') }}" method="POST">
                                            ...
                                            <input type="text" name="coupon"
                                                placeholder="{{ _lang('Enter Coupon Code') }}" required>
                                ...
                                @if (!Cart::isEmpty())
                                    <ul>
                                        @if (get_option('free_shipping_active') == 'Yes' &&
                                            \Cart::getSubTotal() >= get_option('free_shipping_minimum_amount', 0))
                                            <li>
                                                <div class="custom-control custom-radio">
                                                    <input type="radio" id="free_shipping" value="free_shipping"
                                                    ...
                                                    <label class="custom-control-label"
                                                        for="free_shipping">{{ get_option('free_shipping_label') }}</label>
                                                </div>
                                            </li>
                                        @endif
                                        ...
                                        @if (get_option('flat_rate_active') == 'Yes')
                                            <li>
                                                <div class="custom-control custom-radio">
                                                    <input type="radio" id="flat_rate" value="flat_rate"
                                                        name="shipping_method"
                                                        class="custom-control-input select-shipping-method"
                                                        {{ empty(Cart::getCondition(get_option('flat_rate_label'))) ? '' : 'checked' }}>
                                                    <label class="custom-control-label"
                                                        for="flat_rate">{{ get_option('flat_rate_label') }} +
                                                        <b>{!! xss_clean(show_price(get_option('flat_rate_cost'))) !!}</b></label>
                                                </div>
                                            </li>
                                        @endif
                                        ...
                                        @if (get_option('local_pickup_active') == 'Yes')
                                            <li>
                                                <div class="custom-control custom-radio">
                                                    <input type="radio" id="local_pickup" value="local_pickup"
                                                        name="shipping_method"
                                                        class="custom-control-input select-shipping-method"
                                                        {{ empty(Cart::getCondition(get_option('local_pickup_label'))) ? '' : 'checked' }}>
                                                    <label class="custom-control-label"
                                                        for="local_pickup">{{ get_option('local_pickup_label') }} +
                                                        <b>{!! xss_clean(show_price(get_option('local_pickup_cost'))) !!}</b></label>
                                                </div>
                                            </li>
                                        @endif
                                    </ul>
                        ...
                        <div class="single-widget get-button">
                            <div class="content">
                                <div class="button">
                                    <a href="" id="proceed_to_checkout"
                                        class="btn">{{ _lang('Proceed to Payment') }}</a>
                                </div>
                            </div>
                        </div>
                        <!--/ End Button Widget -->
